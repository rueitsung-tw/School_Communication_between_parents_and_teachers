# 報告 agy-0005：將 NVC 四步驟落實至所有 Type B 回覆模组

**執行任務 ID**：0005  
**執行步驟**：唯一步驟 — 更新 README.md 與 11 個 Type B 提示詞模組之 NVC 四步驟內部寫作流程與自然段落表達  
**執行者**：agy  
**執行日期**：2026-08-22  
...
本報告記錄任務 0005 之執行結果。依據派工單 `.codex-orchestration/codex-task-dispatch.md` 與計畫 `.codex-orchestration/plans/plan-0005.md`，本任務將 `README.md` 及 `prompts/` 目錄下全數 11 個 Type B 回覆生成模組，統一將非暴力溝通（NVC）「觀察、感受、需要、請求」轉換為內部寫作思維與步驟，並嚴格規定呈顯給家長時融合為半正式、溫暖、具呼吸感之自然段落。

本任務嚴格遵守權限界線，**僅修改 `README.md`、`prompts/*.md`（11 份包含 Type B 的檔案）與本報告 `report-agy-0005.md`**。未修改任何 Type A 提示詞、Python 程式（`app.py` / `utils.py`）、測試檔（`test_*.py`）、`theme_taxonomy.md`、`research_D_legal.md` 或 `docs/` 文件。

---

## 二、 NVC 四步驟內部化與自然段落設計規範

### 1. NVC 內部寫作四步驟（AI 寫作核心原則）
- **觀察（Observation）**：客觀說明已知／已確認事實與已採取之措施。未經雙方確認之事項承諾查證說明，絕對不可隨意捏造過程、盲目猜測或無條件承諾過失過錯。
- **感受（Feeling）**：真誠同理家長合理的擔心、焦慮或心疼，接住對方情緒，但不替家長診斷、貼標籤或誇大其情緒。
...
| `00_通用_TypeB_回覆草稿生成器.md` | Type B 提示詞 code block | 將「【三段式結構】」更新為「【NVC 非暴力溝通內部寫作流程】」與「【自然段落呈現規範】」，規範觀察不猜測、同理不貼標籤、回應需要與邀請合作下一步。 |
...
| `07_校園安全與意外事故.md` | Type B 提示詞 code block | 內化 NVC 四步驟，說明受傷狀況與關懷不盲目承諾法律責任過失、保留地雷句「小孩子跑跑跳跳難免受傷」提醒，規範自然段落。 |
...
*任務 0005 執行完畢，報告已寫入，停止執行，等待 Codex 審查。*
